...
export const FloatingNav = ({
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
       {!navVisible && (<div className="font-bold md:text-2xl text-xl ">Zija<span className="text-purple">Yaseen</span></div>)
       }
            ...
            <div className="flex flex-col space-y-8 mt-8">
              {navItems.map((navItem, idx) => (
                <Link
                  ...
                  onClick={hideNav}
                  className={cn(
                    "dark:text-neutral-50 text-neutral-600 dark:hover:text-purple hover:text-neutral-500 text-start"
                  )}
                >
                  <span className="text-sm !cursor-pointer">{navItem.name}</span>
                </Link>
